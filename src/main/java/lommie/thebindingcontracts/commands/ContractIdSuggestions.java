...
public class ContractIdSuggestions implements SuggestionProvider<ServerCommandSource> {
    ...
    public CompletableFuture<Suggestions> getSuggestions(CommandContext<ServerCommandSource> commandContext, SuggestionsBuilder suggestionsBuilder) throws CommandSyntaxException {
        try {
            return suggestionsBuilder.suggest(commandContext.getSource().getPlayer().getStackInHand(Hand.MAIN_HAND).get(ModItemComponents.CONTRACT_ID).toString()).buildFuture();
        } catch (Exception e) {
